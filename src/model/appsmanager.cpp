 ...
 * @return false 表示匹配失败
 */
bool AppsManager::fuzzyMatching(const QStringList& AppList, const QString& key)
{
    for (const QString& AppId : AppList) {
        if (key.indexOf(AppId, Qt::CaseInsensitive) != -1) {
            return true;
        }
    }
    return false;
...
QStringList AppsManager::getFilterList()
{
...
void AppsManager::refreshCategoryInfoList()
{
    ...
    QStringList filters = getFilterList();
    ...
    for (const auto &data : datas) {
        bool bContains = fuzzyMatching(filters, data.m_key);
        if (!m_stashList.contains(data) && !bContains) {
            m_allAppInfoList.append(data);
        }
    }
...
void AppsManager::refreshUserInfoList()
{
    ...
    QStringList filters = getFilterList();
    for (auto user=m_userSortedList.begin(); user!=m_userSortedList.end(); user++) {
        if (fuzzyMatching(filters, user->m_key)) {
            m_userSortedList.erase(user);
        }
    }
